...
var payloadServer = net.createServer(function (socket) {
	...
	socket.on('data', function (buf) {
		var timestamp = Math.round(Date.now() / 1000);
		var deviceId = buf.toString('utf8', 0, 8);
		...
		if (buf.length > headerSize + maxFrameSize) {
			log.error({
				...
				payload: buf.slice(headerSize).toString('utf8')
		...
		client.on('error', function (err) {
			log.error({
				type: 'error', timestamp: timestamp,
				addr: addr, deviceId: deviceId, topic: mqttTopic, buffer: buf.toString('utf8'),
				stack: err.stack
			}, '' + err);
		});
		client.on('connect', function () {
			for (var i = 0; i < blocks; i += 1) {
				...
				client.publish(mqttTopic + '/' + deviceId, payload);
			}
